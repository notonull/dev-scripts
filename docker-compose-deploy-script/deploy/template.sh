...
# 基础配置 (统一前缀: base_)
base_image_name="镜像名称:版本"
base_container_name="容器名称"
base_install_path="/opt/server/服务名称"
...
# 安装之后操作
install_after() {
    # if wait_for_service "$base_ip" "端口" 60; then
    #     log_info "$base_container_name 服务启动成功"
    #     # 这里可以添加服务特定的逻辑，比如获取初始密码等
    # else
...
services:
  ${base_container_name}:
    ...
    ports:
      - "端口映射"
    volumes:
      - ${base_install_path}/data:/容器内路径
      - ${base_install_path}/logs:/容器内日志路径
