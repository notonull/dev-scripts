...
# 基础配置 (统一前缀: base_)
base_image_name="sonatype/nexus3:latest"
base_container_name="nexus"
base_install_path="/opt/server/nexus"
...
services:
  ${base_container_name}:
    ...
    ports:
      - "8081:8081"
    volumes:
      - ${base_install_path}/data:/nexus-data
    environment:
      TZ: Asia/Shanghai
...
info() {
    ...
    if [[ "$status" == "running" ]]; then
        log_info "运行时信息:"
        log_info "  ├─ 访问地址: http://${base_ip}:8081/"
        log_info "  ├─ 用户名: admin"
        
        # 初始密码
        local password_file="${base_install_path}/data/admin.password"
        if check_file_readable "$password_file"; then
            ...
            if [[ "$initial_password" != "无法读取" ]]; then
                log_info "  ├─ 初始密码: $initial_password"
            fi
        fi
